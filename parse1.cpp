...
//参数：所在函数名，变量名，类型（普通变量/数组），大小（普通为1，数组为数组大小）
{
	...
	/********设置变量属性********/
	strcpy(thisVar->name, name);
	if(kind != globe)   //不是全局变量，记录所属函数名
	...
	if(kind != globe)
		offset += size;
	varItem* p = varSymbol[varSymbolIndex];
	while (p->next)
	{
...
{   //参数：变量名，存地址的变量，变量所在函数名，变量类型（普通/数组/全局），偏移量（数组要指定，普通变量随意）
	...
	else
	{
		while (p)
		{
			if (strcmp(p->name, name) == 0 && strcmp(p->inFunc, funName) == 0)
			{
				if (kind == array)
...
//int callparameter_list()
//{
//	int es = 0;
//	while (strcmp(token, "NUM") == 0)
//	{
//		es = callparameter_stat();
//		if (es > 0) return es;
//	}
//	return es;
//}
//
//int callparameter_stat()
//{
//	int es = 0;
//	if (strcmp(token, "NUM") == 0)
//	{
...
int globeDeclaration_stat()
{
	...
	if (strcmp(token, ",") == 0)   //标识符之后是逗号,说明这个标识符是普通变量，插入符号表。之后继续声明变量
	{
		es = insert_varSymbol(NULL, name, globe, globeVar);    //全局变量第一个参数为NULL
		++globeVar;
		if (es > 0)
			return es;
		es = declaration_stat();
